...
```{r training}
treeBagDayModel <- caret::train(y = trainingDay$cnt, x = trainingDay[,worthyDayParameters], method = "treebag")
treeBagHourModel <- caret::train(y = trainingHour$cnt, x = trainingHour[,worthyHourParameters], method = "treebag")
```

```{r trainingResdPlotDay}
resdsDay <-  predict(treeBagDayModel, trainingDay[,worthyDayParameters]) - trainingDay$cnt
plot(y = resdsDay, x = trainingDay$dteday)
```

```{r trainingResdPlotHourByDay}
resdsHour <- predict(treeBagHourModel, trainingHour[,worthyHourParameters]) - trainingHour$cnt
