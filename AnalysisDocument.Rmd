...
```{r training}
treeBagDayModel <- caret::train(y = trainingDay$casual, x = trainingDay[,worthyDayParameters], method = "treebag", preProcess = c("scale", "center", "BoxCox"))
treeBagHourModel <- caret::train(y = trainingHour$casual, x = trainingHour[,worthyHourParameters], method = "treebag", preProcess = c("scale", "center", "BoxCox"))
```

```{r trainingResdPlotDay}
resdsDay <-  predict(treeBagDayModel, trainingDay[,worthyDayParameters]) - trainingDay$casual
plot(y = resdsDay, x = trainingDay$dteday)
```

```{r trainingResdPlotHourByDay}
resdsHour <- predict(treeBagHourModel, trainingHour[,worthyHourParameters]) - trainingHour$casual
